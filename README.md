...
~~~
 <span><strong>2.不调用函数：segmentedBitonicSort不调用除标准库函数外的任何其他函数。</strong></span></br>
...
不使用全局变量所以不能引入`#include<cfloat>`来使用`FLT_MAX`表示float范围内的最大值，所有出现FLT_MAX的地方都用`3.40282e+038`代替即可。</br>
 ...
 既然不能调用标准库函数`is_nan`来判断一个数是否是NaN，那我们自己写一个if语句来判断：</br>
 ~~~cpp
...
----------------------------
4月22日收到邮件之后大概看了一下bitonic sort算法的讲解就去复习今天的一门专业课考试了。今天从晚上6点开始查资料编程到现在正在写文档，大概用了将近3个小时。
